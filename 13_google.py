...
# 최신 GoogleGenerativeAI 대신 ChatGoogleGenerativeAI 사용
chat = ChatGoogleGenerativeAI(
    model=os.getenv('BASIC_GEMINI_MODEL'),
...
memory = ConversationSummaryBufferMemory(
    llm = chat, # LLM 설정
    max_token_limit=160, # 데이터 저장 Max 값 설정
    return_messages = True, # 결과값을 출력할지 설정, 기본 false 
)

prompt = ChatPromptTemplate.from_messages([
    ("human", "You are a helpful AI talking to a human. Please respond to the following question: {question}"),
...
# 참고 : https://velog.io/@udonehn/LangChain%EC%9D%84-%EC%82%AC%EC%9A%A9%ED%95%98%EC%97%AC-%EC%B1%97%EB%B4%87-%EB%B0%98%EB%93%A4%EA%B8%B0#%EC%B2%BC%EC%9D%B8%EC%9D%98-%ED%9D%90%EB%A6%84
